...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    if (cli_socket < 0){
        perror("start client");
        return client_cleanup(cli_socket, (char *)cmd_buff, rsp_buff, ERR_RDSH_CLIENT);
    }

    while (1) {
        printf(SH_PROMPT);
        if(!fgets(rsp_buff, SH_CMD_MAX, stdin)){
    ...
    return client_cleanup(cli_socket, (char *)cmd_buff, rsp_buff, OK);
}
 ...
 * 
 *   returns:
 *          rc:          return the provided error code
 * 
 */
