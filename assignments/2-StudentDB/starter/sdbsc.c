...
int get_student(int fd, int id, student_t *s){
    ...
    student_t storageStudent;
    ssize_t bytes;
    while((bytes = read(fd, &storageStudent, sizeof(student_t))) > 0){
        if(storageStudent.id == id){
            memcpy(s, &storageStudent, sizeof(student_t));
            return NO_ERROR;
        }
    }
    if(bytes < 0){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    ssize_t bytes = write(fd, &newStudent, sizeof(student_t));
    if(bytes != sizeof(student_t)){
...
int del_student(int fd, int id){
    ...
    student_t emptyStudent = EMPTY_STUDENT_RECORD;
    ssize_t bytes = write(fd, &emptyStudent, sizeof(student_t));
